...
def count_of_nodes(root, queries, s):
  ...
  while queries:
    val, char = queries.pop(0)
    #print(val, char)
    queue = [(1, root)]
    count = 0

    while queue:
      subroot, curr = queue.pop(0)
      if subroot == val: 
        if s[curr.val - 1] == char:
          #print(subroot, curr.val)
          count += 1

      for i in range(len(curr.children)):
        if subroot == val:
          queue.append((subroot, curr.children[i]))
        else:
          queue.append((curr.children[i].val, curr.children[i]))
...
if __name__ == "__main__":
  ...
  root_2.children[0].children[1].children.append(Node(8))
